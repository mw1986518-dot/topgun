...
function makeActionPlanQuestion(
  overrides: Partial<ActionPlanQuestion> = {},
): ActionPlanQuestion {
  return {
    key: "budget",
    question: "月薪范围是多少？",
...
describe("ConsensusOutput", () => {
  ...
  it("renders consensus content as markdown", () => {
    render(<ConsensusOutput content="这是一份测试报告内容" toleratedRisks={[]} />);
  ...
  it("hides risk section when there are no risks", () => {
    ...
    expect(
      screen.queryByText("容忍风险清单（临时接受，非最终共识）"),
  ...
  it("renders header title", () => {
    ...
    expect(screen.getByText("阶段 4：最终共识与交付")).toBeInTheDocument();
  });
  ...
  it("shows error when trying to export empty/whitespace content", async () => {
    ...
    expect(await screen.findByText("当前没有可导出的内容")).toBeInTheDocument();
  });
